...
public class MarketplaceFuncionalidade {
	...
	public MarketplaceFuncionalidade() {
		...
		AnuncioRevendaRepositorioMemoria anuncioRepo = new AnuncioRevendaRepositorioMemoria(ingressoRepo);
	...
	static class EventoRepositorioMemoria implements EventoRepositorio {
		...
		public List<Evento> listarPorOrganizador(UsuarioId organizadorId) {
			List<Evento> resultado = new ArrayList<>();
			for (Evento e : dados.values()) {
				if (e.getOrganizadorId().equals(organizadorId)) {
					resultado.add(e);
				}
			}
			return resultado;
	...
	static class IngressoRepositorioMemoria implements IngressoRepositorio {
		...
		public List<Ingresso> pesquisarPorTipo(TipoIngressoId tipoIngressoId) {
			List<Ingresso> resultado = new ArrayList<>();
			for (Ingresso i : dados.values()) {
				if (i.getTipoIngressoId().equals(tipoIngressoId)) {
					resultado.add(i);
				}
			}
			return resultado;
	...
	static class AnuncioRevendaRepositorioMemoria implements AnuncioRevendaRepositorio {
		...
		AnuncioRevendaRepositorioMemoria(IngressoRepositorio ingressoRepositorio) {
		...
		public boolean existeDisponivelOuReservadoParaEvento(EventoId eventoId) {
			for (AnuncioRevenda a : dados.values()) {
				...
				for (IngressoId iid : a.getIngressoIds()) {
					Ingresso ing = ingressoRepositorio.obter(iid);
					if (ing.getEventoId().equals(eventoId)) {
						return true;
					}
				}
			}
		...
		public List<AnuncioRevenda> pesquisarPorEvento(EventoId eventoId) {
			...
			for (AnuncioRevenda a : dados.values()) {
				for (IngressoId iid : a.getIngressoIds()) {
					if (ingressoRepositorio.obter(iid).getEventoId().equals(eventoId)) {
						resultado.add(a);
						break;
					}
				}
