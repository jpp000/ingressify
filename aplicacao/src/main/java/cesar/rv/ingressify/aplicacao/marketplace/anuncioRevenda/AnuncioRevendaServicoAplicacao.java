...
public class AnuncioRevendaServicoAplicacao {
	...
	public AnuncioRevendaServicoAplicacao(AnuncioRevendaServico anuncioRevendaServico,
			...
			IngressoServico ingressoServico, EventoRepositorio eventoRepositorio,
			UsuarioRepositorio usuarioRepositorio, PagamentoServico pagamentoServico,
			SaldoServico saldoServico, TransacaoServico transacaoServico, UsuarioServico usuarioServico) {
	...
	public AnuncioRevenda obter(AnuncioRevendaId anuncioId) {
		return anuncioRevendaServico.obter(anuncioId);
	}

	public List<AnuncioRevenda> listarPorEvento(EventoId eventoId) {
	...
	public void confirmarCompra(AnuncioRevendaId anuncioId) {
		...
		transacaoServico.registrar(new Transacao(a.getVendedor(), TipoTransacao.VENDA,
				divisao.vendedor(), agora, pagamento.getCorrelacaoId()));
		transacaoServico.registrar(new Transacao(comprador, TipoTransacao.COMPRA,
				pagamento.getValor(), agora, pagamento.getCorrelacaoId()));
	}
