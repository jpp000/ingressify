...
public class IngressoController {
	...
	public IngressoController(CompraServicoAplicacao compraServico, IngressoServicoAplicacao ingressoServico,
			ReembolsoServicoAplicacao reembolsoServico, IngressoRepositorio ingressoRepositorio) {
	...
	public ResponseEntity<List<IngressoResponse>> comprar(
		...
		try {
			...
			List<IngressoResponse> resp = ids.stream()
					.map(id -> IngressoResponse.fromDomain(ingressoRepositorio.obter(id)))
					.toList();
			return ResponseEntity.status(HttpStatus.CREATED).body(resp);
		} catch (IllegalStateException e) {
			if (e.getMessage() != null && e.getMessage().contains("Saldo")) {
	...
	public ResponseEntity<List<IngressoResponse>> meus(
			@RequestHeader("X-Usuario-Id") int usuarioId) {
		List<IngressoResponse> resp = ingressoRepositorio
	...
	public ResponseEntity<IngressoResponse> detalhe(@PathVariable String id) {
		try {
			var ingresso = ingressoRepositorio.obter(new IngressoId(UUID.fromString(id)));
			return ResponseEntity.ok(IngressoResponse.fromDomain(ingresso));
	...
	public ResponseEntity<String> qrcode(@PathVariable String id) {
		try {
			ingressoRepositorio.obter(new IngressoId(UUID.fromString(id)));
